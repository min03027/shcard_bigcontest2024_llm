...
def generate_response_with_faiss(question, df, embeddings, model, embed_text, time, local_choice, index_path=os.path.join(module_path, 'faiss_index.index'), max_count=10, k=3, print_prompt=True):
    ...
    # 영업시간 및 현지인 맛집 조건 필터링
    # ... (영업시간 필터링 로직은 기존 코드와 동일)
    ...
    if local_choice == '제주도민 맛집':
        local_choice_text = '제주도민(현지인) 맛집'
    else:
        local_choice_text = '관광객 맛집'

    # 데이터에서 참조 정보를 구성
    reference_info = ""
    for idx, row in filtered_df.iterrows():
        reference_info += f"식당명: {row['가맹점명']}, 가맹점업종: {row['가맹점업종']}, 이용건수구간: {row['이용건수구간']}\n"

    # 응답을 받아오기 위한 프롬프트 생성
    prompt = (f"질문: {question}\n"
              f"특히 {local_choice_text}을 선호해.\n"
              f"다음은 관련 정보입니다:\n{reference_info}\n"
              "응답:")
